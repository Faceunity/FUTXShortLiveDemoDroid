...
import com.tencent.qcloud.ugckit.utils.PreferenceUtil;
...
public class UGCKitVideoRecord extends AbsVideoRecordUI implements
        ...
        RecordBottomLayout.OnSwitchCameraListener,
        VideoRecordSDK.OnVideoRecordListener, SensorEventListener {
    ...
    private void initDefault(Context context) {
        ...
        getRecordBottomLayout().setOnSwitchCameraListener(this);
    ...
    public void onSwitchCamera(boolean isFront) {
        skipFrame = 5;
        if (mFURenderer != null) {
            mFURenderer.setCameraFacing(isFront ? CameraFacingEnum.CAMERA_FRONT : CameraFacingEnum.CAMERA_BACK);
...
//                        getRecordMusicPannel().setMusicName("");
    ...
    private void initFURender() {
        String isOpen = PreferenceUtil.getString(getContext(), PreferenceUtil.KEY_FACEUNITY_ISON);
        ...
        Log.d(TAG, "initFURender: " + mIsOpenFuBeauty);
        if (mIsOpenFuBeauty) {
    ...
    private FURendererListener mFURendererListener = new FURendererListener() {
        ...
        public void onTrackStatusChanged(FUAIProcessorEnum type, int status) {
            final FUAIProcessorEnum finalType = type;
            final int finalStatus = status;
            ThreadUtils.getUiThreadHandler().post(new Runnable() {
                @Override
                public void run() {
                    if (mTvFace != null) {
                        mTvFace.setText(finalType == FUAIProcessorEnum.FACE_PROCESSOR ? R.string.toast_not_detect_face : R.string.toast_not_detect_body);
                        mTvFace.setVisibility(finalStatus > 0 ? View.INVISIBLE : View.VISIBLE);
                    }
                }
            });
        }
    ...
    private void registerVideoProcessListener() {
        ...
        Log.d(TAG, "initFURender onTextureCustomProcess: " + instance);
        instance.setVideoProcessListener(new TXUGCRecord.VideoCustomProcessListener() {
            ...
            public int onTextureCustomProcess(int textureId, int width, int height) {
                Log.d(TAG, "initFURender onTextureCustomProcess: " + textureId + ", mIsOpenFuBeauty: " + mIsOpenFuBeauty);
                if (!mIsOpenFuBeauty) {
                    return textureId;
                }
...
//                return textureId;
                ...
                return mFURenderer.onDrawFrameSingleTex(textureId, width, height);
            }
